...
export default function MatchScreenNode({ id, data, selected }) {
  ...
  return (
    <NodeBase
      selected={selected}
      isActive={isActive}
      categoryClass="node-condition"
    >
      <NodeHeader
        icon={<ImageIcon size={13} />}
        label={data.label || 'Match Screen'}
      ...
      <div className="px-3 py-2 space-y-1 bg-bg-primary bg-opacity-40">
        ...
        {result && (
          <div className={`text-xs font-semibold ${result.output === 'match' ? 'text-accent-green' : 'text-accent-red'}`}>
      ...
      <Handle
        ...
        id="match"
        style={{ background: '#22c55e', border: '2px solid #16a34a', top: '35%' }}
      />
      <Handle
        type="source"
        position={Position.Right}
        id="no_match"
        style={{ background: '#ef4444', border: '2px solid #dc2626', top: '65%' }}
      />

      <div className="absolute right-3 flex flex-col gap-3 pointer-events-none" style={{ top: '28%' }}>
        <span className="text-[9px] text-accent-green leading-none">match</span>
      </div>
      <div className="absolute right-3 flex flex-col gap-3 pointer-events-none" style={{ top: '58%' }}>
        <span className="text-[9px] text-accent-red leading-none">no match</span>
      </div>
    </NodeBase>
  );
}
