...
export default function WaitForImageNode({ id, data, selected }) {
  ...
  return (
    ...
    >
      <NodeHeader
        icon={<Clock size={13} />}
        label={data.label || 'Wait for Image'}
      ...
      <div className="px-3 py-2 space-y-1 bg-bg-primary bg-opacity-40">
        ...
        <NodeProp label="Timeout" value={data.timeout !== undefined ? `${data.timeout}s` : '30s'} />
        {result && (
          <div className={`text-xs font-semibold ${result.output === 'found' ? 'text-accent-green' : 'text-accent-orange'}`}>
      ...
      <Handle
        ...
        position={Position.Right}
        id="timeout"
        style={{ background: '#f97316', border: '2px solid #ea580c', top: '65%' }}
      />
      ...
      <div className="absolute right-3 pointer-events-none" style={{ top: '58%' }}>
        <span className="text-[9px] text-accent-orange leading-none">timeout</span>
      </div>
    </NodeBase>
  );
}
